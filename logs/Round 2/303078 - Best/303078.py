...
class Trader:
    ...
    OSM_INV_SWEEP_SCALE = 3       # ← from 4
    OSM_BUY_EDGE        = 0       # unchanged
    OSM_SELL_EDGE        = 4      # ← from 5
    OSM_SKEW            = 4       # ← from 6  (KEY CHANGE)
    OSM_PASSIVE_CAP     = 20      # ← from 15
    ...
    PEP_SKEW          = 3
    PEP_TARGET        = 70
    PEP_BUILD_AGGR    = 2         # ← from 1
    PEP_SELL_THR      = 3         # ← from 2
    PEP_HOLD_THR      = 4
    PEP_HOLD_SELL_QTY = 5
    ...
    def _trade_osmium(self, od, pos, lim):
        ...
        # ── inventory mid-sell / mid-buy (EXPANDED to FV+1) ──
        ep = pos + bu - su
        if ep > 0:
            # Sell at FV+1 first (better price), then FV
            ...
            if fv in od.sell_orders and bu < mb:
                q = min(-od.sell_orders[fv], mb - bu,
                        max(0, -(pos + bu - su)))
                if q > 0:
