...
class Trader:
    ...
    OSM_EMA_ALPHA = 0.01
    OSM_MR_WEIGHT = 0.3
    OSM_FV_CLAMP  = 4             # widened from 2 to give flow signals room

    # ── Osmium: order-flow params ────────────
    OSM_NPC_SHIFT_ALPHA = 0.15    # EMA smoothing on NPC mid deltas
    OSM_NPC_SHIFT_COEFF = 0.25     # 1-tick NPC shift → 0.5 FV adjustment
    OSM_TFLOW_ALPHA     = 0.3     # EMA smoothing on trade-flow
    OSM_TFLOW_COEFF     = 0.04    # per-lot aggressive flow → FV impact
    OSM_DEPLETION_COEFF = 0.3     # FV nudge when a whole book side is gone

    # ── Osmium: trading ──────────────────────
    OSM_SWEEP_SELL_EDGE = 1
    OSM_INV_SWEEP_SCALE = 1.5
    OSM_BUY_EDGE        = -1
    OSM_SELL_EDGE        = 3
    OSM_SKEW            = 2
    OSM_PASSIVE_CAP     = 35
    ...
    def _trade_osmium(self, od, pos, lim, fv):
        ...
        bpx = min(bpx_raw, fv + 2)      # allow bidding above fv
        spx = max(spx_raw, fv)
        ...
        # Level 1: tight (50% of volume)
        if rb > 0:
            q1 = max(1, rb // 2)
            orders.append(Order(S, int(bpx), q1))
            rem_b = rb - q1

            # Level 2: one tick wider (30%)
            q2 = max(1, min(rem_b, rb * 3 // 10))
            if q2 > 0:
                orders.append(Order(S, int(bpx - 1), q2))
                rem_b -= q2

            # Level 3: two ticks wider (20%) — catches bigger moves
            if rem_b > 0:
                orders.append(Order(S, int(bpx - 2), rem_b))
        ...
        if rs > 0:
            ...
            q2 = max(1, min(rem_s, rs * 3 // 10))
            if q2 > 0:
                orders.append(Order(S, int(spx + 1), -q2))
                rem_s -= q2

            if rem_s > 0:
                orders.append(Order(S, int(spx + 2), -rem_s))
