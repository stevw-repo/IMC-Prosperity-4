...
class Trader:
    ...
    def _trade_osmium(self, od, pos, lim):
        ...
        # ────────────────────────────────────────────
        # FIX 2+3: inventory-scaled sweep thresholds
        # ────────────────────────────────────────────
        #  nonlinear curve: |r|*(1+|r|) → 0 at flat, 0.75 at half, 2 at full
        short_pen = round(max(0, -ratio) * (1 + max(0, -ratio))
                         * self.OSM_INV_SWEEP_SCALE)
        long_pen  = round(max(0,  ratio) * (1 + max(0,  ratio))
                         * self.OSM_INV_SWEEP_SCALE)

        #  flat:    sell at 10002+,  buy at ≤10000
        #  short40: sell at 10005+,  buy at ≤10000
        #  long40:  sell at 10002+,  buy at ≤9997
        sell_sweep_min = fv + self.OSM_SWEEP_SELL_EDGE + short_pen
        buy_sweep_max  = fv - long_pen

        # ═══ L1: sweep clearly mispriced levels ═══
        ...
        # ═══ L1b: take at FV to flatten inventory ═══
        ep = pos + bu - su
        ...
        spx_raw = fv + self.OSM_SELL_EDGE - sk     # FIX 1: SELL_EDGE=3

        # ────────────────────────────────────────────
        # FIX 4: floor/ceiling at FV, not FV∓1
        #   When short → skew pushes bid UP toward FV (faster unwind)
        #   When long  → skew pushes ask DOWN toward FV (faster unwind)
        #   When flat  → bid=9998, ask=10003 (no crossing)
        # ────────────────────────────────────────────
        bpx = min(bpx_raw, fv)
        spx = max(spx_raw, fv)

        # ────────────────────────────────────────────
        # FIX 5: cap passive qty on inventory-building side
        # ────────────────────────────────────────────
        if pos < -25:
